...
function getOperations(string){
    ...
    for (let i = 0; i < string.length; i++) {
        if (isNaN(string[i]) && string[i] != '.'){ //check this is a operation symbol
            operations.push({
                symbol: string[i],
...
function getNumbers(string) {
    ...
    for (let i = 0; i < string.length; i++) {
        if (!isNaN(string[i]) || string[i] == '.'){ //check this is a digit or .
...
function executeSimpleExpression(input) {
    ...
    for (let i = 0; i < operations.length; i++) {
        ...
        numbers[operation.positionsOfValues[0]] = simpleCalc(value1, value2, operation.symbol)
        numbers.splice(operation.positionsOfValues[1], 1)

        for (let t = 0; t < operations.length; t++) {
            if (operations[t].positionsOfValues[0] >= operation.positionsOfValues[1]) {
...
input = input.replace('(' + simpleExpressions[i] + ')', executeSimpleExpression(simpleExpressions[i]))
}
